...
*/
  ...
  //################################################################
  std::array<double, 8> lat{ 44.781003, 44.780996, 44.781062, 44.781068, 44.781133, 44.781129, 44.781194, 44.781198};  //44.781715, 44.781720,
  std::array<double, 8> lon{ 8.639942, 8.641018, 8.641000,8.639904, 8.639889, 8.640959, 8.640930, 8.639865}; //8.640793, 8.640425,
...
void publish_command_position_to_drone(Drone *drone)
{
  ...
  if (mission.inNavigation == true)
  {
    ...
    // verifica se la longitudine del secondo waypoint è stata superata. In quel cas il drone si ferma.
...
void navigation(Drone *drone, KF *Kalman_Filter, PID *pid_x, PID *pid_y, PID *pid_z, PID *pid_yaw)
{
  ...
  /* SIMULATION CASE */
  if (first_time == true && mission.use_two_single_fixed_waypoints_flag == false)
  {
    if (drone-> yaw_in_Rad > M_PI - M_PI/3 && drone-> yaw_in_Rad < -M_PI + M_PI/3)
  {
    drone-> yaw_des_rad = M_PI;
  }
  else
  {
     drone-> yaw_des_rad = 0.00;
  }
  //Ev
  }
...
*/
 ...
 // drone->yaw_rate_cmd = pid_yaw->position_control_knowing_velocity(drone->yaw_des_rad , drone->yaw_in_Rad, 0, drone->yaw_vel);
  ...
  drone->x_pos_cmd = 2.0*(drone->x_target - drone->current_local_pos.x);
...
void jump_panels(Drone *drone, PID *pid_x, PID *pid_y, PID *pid_z, PID *pid_yaw)
{
  ...
  else
  {
    ...
    drone->x_vel_cmd = pid_x->position_control_knowing_velocity(mission.target_offset_x, drone->current_local_pos.x, 0, drone->current_local_velocity.vector.x);
    drone->y_vel_cmd = pid_y->position_control_knowing_velocity(mission.target_offset_y, drone->current_local_pos.y, 0, drone->current_local_velocity.vector.y);
    drone->z_vel_cmd = pid_z->position_control_knowing_velocity(mission.target_offset_z, drone->current_local_pos.z, 0, drone->current_local_velocity.vector.z);
...
void conduct_MATRICE_on_first_waypoint(Drone *drone)
{
  bool obtain_control_result = false;
  bool release_user_control = false;
  waypoints.counter = 0;

  if (mission.counter_case_9 == 0)
  {
    obtain_control_result = drone->release_control();
    if (!obtain_control_result)
    {
  ...
  // Drone rimane in questo caso finche il pulsante non viene premuto nuovamente
  //drone.key_for_control_is_pressed --> flag relativo al fatto se un tasto viene premuto
  cout << "[CONDUCT MATRICE FIRST WAY] THE CONTROL IS ON THE USER! KEY PRESSED:  " << drone->array_number<< endl;

  double P1_lat = waypoints.GPS_waypoints_lat[0];
  double P1_lon = waypoints.GPS_waypoints_lon[0];

  evaluate_local_offset_between_GPS_target_and_body_frame(P1_lat, P1_lon, drone->current_gps, drone->current_local_pos, drone->target_offset_respect_drone_position);
  float distance = sqrt(pow( drone->target_offset_respect_drone_position.x,2) + pow( drone->target_offset_respect_drone_position.y,2));

  cout << "DISTANCE FROM FIRST WAYPOINT: " << distance << endl;  
  //If a key is pressed the UAV verify if the distance between its position and the First Waypoint is under a certain threshold.
  //If Yes it starts the Autonomous Navigation
  
  cout << "drone->key_for_control_is_pressed: " << drone->key_for_control_is_pressed << endl;
  cout << " mission.counter_case_9: " <<  mission.counter_case_9 << endl;

  bool flag_start_mission = false;
  if (drone->key_for_control_is_pressed == true && mission.counter_case_9 > 100)
  {
     //Verify the distance between the UAV and the Start Waypoint 
     ...
     {
        ROS_ERROR("Impossible to Obtain Drone Control by OSDK. Conduct the drone at least two meters near the Start Waypoint");
  ...
  { 
    ...
    {
        if (drone->key_for_control_is_pressed == true &&  release_user_control == true)
        {
          //Sleep for 3 second until the flag becomes false, otherwise the code come back here from navigation case
          cout << "[USER CONTROL TASK] WAIT THREE SECONDS TO START EXECUTION. Switch to Nav Mode" << endl;
          sleep(3);
  ...
  else
  {
    ...
    cout << "SONO QUI" << endl;
    if (mission.counter_case_9 > 100)
    {
      cout << "YOU CAN NOW PRESS A KEY!:  " << endl;
      cout << "PLACE THE DRONE CORRECTLY ON THE FIRST ARRAY" << endl;
    }
    //Continua  a rimanere in questo caso ficnhe un tasto non viene premuto
    mission.state = 9;
    mission.counter_case_9 = mission.counter_case_9 + 1;
    drone->user_control_flag = true;
  }
}
...
int main(int argc, char **argv)
{
  ...
  nh.getParam("/solar_param/autonomous_mission_with_manual_take_off", mission.start_autonomous_mission );
...
string stringpath = "/home/dji/DATA/dji_ws/simulation_data/test_Predosa/TEST_4/" + folder_name + "/";
...
if (status != 0)
{
  ...
  while (nh.ok())
  {
    ...
    */
    ...
    //Caso relativo all'utilizzo della modalità completamente autonoma ma permettendo all'utente di pèosizionare il drone sul primo array. 
    //Implementata solo per i test di Predosa 
    case 9:
      conduct_MATRICE_on_first_waypoint(&drone);
